...
<html lang="en">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
   ...
   @endif
    <form   action="{{url('/buscar')}}" method="post" class="form-inline my-2 my-lg-0">
    {{ csrf_field()}}
      <input class="form-control mr-sm-2  @error('buscar') is-invalid @enderror" type="search" placeholder="Buscar" name="buscar"aria-label="Search" required>
...
<div class="cuerpo_buscar">

<div class="foto_buscar">
        <img src="{{asset('imagenes_recursos/'.$recurso->foto)}}" alt="">
</div>
<div class="info_buscar">

<div class="info_buscar_cabeza">
    <div class="info_buscar_cabeza1"></div>
    <h4>{{ $recurso->tema }}</h4>
    <div class="info_buscar_cabeza2">
        <h4>vistos {{ $recurso->vistas}}</h4>
    </div>
</div>
<div class="info_buscar_cuerpo">
<h4>{{ $recurso->nombre}}</h4>
<h4>{{ $recurso->descripcion}}</h4>
</div>
<div class="info_buscar_pie">
 @if (Auth::check())
                   <form action="{{url('/estudiante/recurso/'.$recurso->id)}}">
                    ...
                    @else
                    <form action="{{url('/')}}">
                        <button class="info_buscar_pie" type="submit" onclick="return confirm('Inicia sesion primeros')">Ver</button>
                    </form>
                    @endif
</div>
